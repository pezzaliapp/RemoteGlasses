/* RemoteGlasses — MVP WebRTC + WebSocket signaling (MIT)
   Nota: per produzione aggiungere un TURN server (coturn) e usare HTTPS/WSS.
*/
...
window.installPWA = () => alert('Installa come App: usa “Aggiungi a Home” dal browser.');
...
function makePeer(){
  pc = new RTCPeerConnection({
    iceServers: [
      ...
      // TURN esempio (attivalo in produzione):
      // { urls: 'turn:TUO_IP_O_DOMINIO:3478', username: 'user', credential: 'pass' }
    ]
  });
...
function connectWS(){
  return new Promise((resolve, reject) => {
    const url = wsUrlEl.value || 'ws://localhost:8080';
...
async function start(){
  ...
  const wsUrl = wsUrlEl.value || 'ws://localhost:8080';
